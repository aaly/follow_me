...
class IBeaconAdvertised: public BLEAdvertisedDeviceCallbacks {
  ...
  Result<std::string> BluetoothService::Init(const ParametersPack& config) {
    ...
    /*AddCallback(std::string("scan"), [&](const ParametersPack& data) { 
      Scan();
      return Result<std::string>();
      });


      AddCallback(std::string("run"), [&](const ParametersPack& data) { 
      if(data.GetParameter<std::string>("type") == "scanner") {
        return Scan();
      }
      return Result<std::string>();
      });

      AddCallback(std::string("scanner_subscribe"), [&](const ParametersPack& data) { 
        return Subscribe(data.GetParameter<std::string>("name"));
      });*/
  ...
  Result<std::string> BluetoothService::Run() {
    Result<std::string> result;
  ...
  Result<std::string> BluetoothService::Subscribe(const std::string& server_name) {
    Result<std::string> result;
